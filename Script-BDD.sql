...
insert into UserB (UName, UPassword, UNameCo, UEmail, UDate, UType, UPictureID,FCounter,DCounter) 
		values ('juan','123','usuario Universal','jaja@gmail.com','1998-12-05','U',1,0,0);
CREATE USER juan@localhost IDENTIFIED BY '123';
GRANT 'usuarios' TO 'juan'@'localhost';
SET DEFAULT ROLE usuarios TO juan@localhost;

insert into UserB (UName, UPassword, UNameCo, UEmail, UDate, UType, UPictureID,FCounter,DCounter) 
		values ('adminU','Cookies','Administrador Universal','alejandromedinamx@gmail.com','1998-02-05','A',1,0,0);
CREATE USER adminU@localhost IDENTIFIED BY 'Cookies';
GRANT administradores TO adminU@localhost;
SET DEFAULT ROLE administradores TO adminU@localhost;
...
DELIMITER //
create trigger nuevo_favorito after insert on Favorites
for each row
begin
	update UserB
	set FCounter=FCounter+1
	where UName=New.FUName;
end
//

DELIMITER //
create trigger favorito_eliminado after delete on Favorites
for each row
begin
	update UserB
	set FCounter=FCounter-1
	where UName=OLD.FUName;
end
//

DELIMITER //
create trigger libro_descargado after insert on History
for each row
begin
	update UserB
	set DCounter=DCounter+1
	where UName=NEW.HUName;
end
//
